...
echo "$($PSQL "SELECT to_char(AVG(winner_goals),'9.99') FROM games")"
...
echo "$($PSQL "SELECT name FROM teams AS t INNER JOIN games AS g ON t.team_id = g.winner_id WHERE g.round = 'Final' AND g.year = 2018 ")"
...
echo "$($PSQL "SELECT year, name FROM teams AS t INNER JOIN games AS g ON t.team_id = g.winner_id WHERE g.round = 'Final' ORDER BY g.year")"
